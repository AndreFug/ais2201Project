...
#define ADC_sample_rate 10000.0f
#define const NUM_TAPS 51
...
float32_t fir_state[NUM_TAPS + WINDOW_SIZE];
...
void apply_hanning_window(float32_t *buffer, uint16_t size);
void zero_pad_signal(float32_t *buffer, uint16_t size, uint16_t target_size);
void initialize_fir_filter(float32_t cutoff_low, float32_t cutoff_high);


void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
	read_buffer_start = WINDOW_SIZE ;
...
void apply_hanning_window(float32_t *buffer, uint16_t size) {
    for (int i = 0; i < size; i++) {
        buffer[i] *= 0.5f * (1.0f - arm_cos_f32((2.0f * M_PI * i) / (size - 1)));
    }
}

void zero_pad_signal(float32_t *buffer, uint16_t size, uint16_t target_size) {
    for (int i = size; i < target_size; i++) {
        buffer[i] = 0.0f;
    }
}

void initialize_fir_filter(float32_t cutoff_low, float32_t cutoff_high) {
    float32_t normalized_cutoff[2] = {cutoff_low / (ADC_SAMPLING_FREQ / 2), cutoff_high / (ADC_SAMPLING_FREQ / 2)};
    arm_fir_init_f32(&S, NUM_TAPS, fir_coeffs, fir_state, WINDOW_SIZE);
    arm_fir_design_bandpass(normalized_cutoff, NUM_TAPS, fir_coeffs);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  initialize_fir_filter(25.0f, 4200.0f);
  arm_rfft_fast_init_f32(&rfft_instance, WINDOW_SIZE);
  /* USER CODE END Init */
  ...
  while (1)
  {
    if (buffer_full == 1) {
        ...
        for (int i = 0; i < WINDOW_SIZE; i++) {
          ...
          apply_hanning_window(RFFT_Input, WINDOW_SIZE);

          /* Zero padding */
          zero_pad_signal(RFFT_Input, WINDOW_SIZE, ADC_BUFFER_SIZE);

          /* FIR filter */
          float32_t filtered_signal[WINDOW_SIZE];
          arm_fir_f32(&S, RFFT_Input, filtered_signal, WINDOW_SIZE);
